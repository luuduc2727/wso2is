...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        // Drawer dropdown (responsive): same init so it shows current language and opens on click
        const drawerDropdown = $("#language-selector-dropdown-drawer");
        if (drawerDropdown.length) {
            const drawerInput = $("#language-selector-input-drawer");
            const drawerSelectedText = $("#language-selector-selected-text-drawer");
            drawerDropdown.dropdown('hide');
            $("> input.search", drawerDropdown).attr("role", "presentation");
            drawerInput.val(userLocaleFromLocalize);
            const drawerDataOption = drawerDropdown.find("div.item[data-value='" + userLocaleFromLocalize + "']");
            drawerDataOption.addClass("active selected");
            drawerSelectedText.removeClass("default");
            drawerSelectedText.html(drawerDataOption.html());
        }
    });
     ...
     * @param inputEl - Optional: the hidden input that changed (e.g. from drawer dropdown). If omitted, uses header input.
     */
    function onLangChange(inputEl) {
        const langSwitchForm = (inputEl && typeof inputEl.value !== 'undefined') ? inputEl : document.getElementById("language-selector-input");
        if (!langSwitchForm) return;
